...
const { listRoom, drawData, layer, layerStorage } = require("./globalVar");
...
// io.on("connection", (socket) =>webSocket(socket,io))
io.on("connection", (socket) =>webSocket(socket,io))

// io.on("connection", function (socket) {
//    // console.log("🚀 ~ socket", socket.objectID)
...
//          drawData.push(data);
//          // console.log(`  ~ drawData`, drawData.length);
...
//          io.to(room).emit("fetch-data-to-client", { drawData, layer, layerStorage });
//       });
...
//          deleteObjInPool(data.objectID, drawData, data.layer);
...
//          if (data.objectID) {
//             updateObjectByID(drawData, data);
//          }
...
//          turnOnOffUsernamePoolData(data.userID, drawData, data.name);
